...
\newcounter{spuddy}
\def\spudlemon{%
  \ifnum\c@spuddy=0{lemon}%
  \else{\addtocounter{spuddy}{-1}spud-\spudlemon}%
  \fi}
\def\spudslemon#1{\setcounter{spuddy}{#1}\spudlemon}
\renewcommand{\thepart}{\setcounter{spuddy}{\the\c@part}\spudlemon}
\makeatother
...
\newcommand{\D}[1]{\blue{\mathtt{#1}}}
\newcommand{\C}[1]{\red{\mathtt{#1}}}
\newcommand{\F}[1]{\green{\mathtt{#1}}}
\newcommand{\V}[1]{\purple{\mathtt{#1}}}
\newcommand{\K}[1]{\underline{\mathrm{#1}}}

\newcommand{\ask}{\mathtt{ask}}
\newcommand{\cmt}[1]{\texttt{--}\;\mbox{#1}}

\newcommand{\Number}{\D{Number}}
\newcommand{\Ze}{\C{Z}}
\newcommand{\Su}{\C{S}}
...
\newtheorem{askism}{Askism}[chapter]


\title{Mathematical Structure for Programming}
\author{Conor McBride and maybe you, too}
\maketitle

%\renewcommand{\thepart}{\arabic{part}}
\setcounter{part}{-1}

\part{the difference engine}
...
Find a piece of paper and a pencil, or a pen if you feel confident. Somewhere a little below and to the right of the top left corner (we'll use those gaps later), write the three numbers 0, 1, 2 in a diagonal line, like this
\[\begin{array}{cc@{}c@{}c}
\ulcorner \\
&0 &   & \\
&  & 1 & \\
...
\end{array}
\hspace*{5in}\]
We're all set to compute like a machine. You're probably not a machine, but before you ask machines to compute, you should have the decency know what it is you are asking of them. To make a step in this computation, you write another parallel diagonal stripe of numbers, starting to the right of the 0 and above the 2. Compute the numbers in the new stripe by adding the numbers from the old stripe coming from left and below-left. In the bottom row, there is nothing below-left, so imagine that the missing number is 0.
\newcommand{\upri}{{}^\nearrow\hspace*{-0.15in}\rightarrow }
\[\begin{array}{lr@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner&\\
& 0\, & \rightarrow & 1     &       &   & \mbox{(because $0+1=1$)}\\
&     & 1           & \upri & 3     &   & \mbox{(because $1+2=3$)}\\
&     &             & 2     & \upri & 2 & \mbox{(because $2+0=2$)}
\end{array}
\hspace*{5in}\]

Now take another step, following the same procedure.
\[\begin{array}{lr@{}r@{}r@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner&\\
& 0\, &   & 1 & \rightarrow  & 4     &       &   & \mbox{(because $1+3=4$)}\\
&     & 1 &   & 3            & \upri & 5     &   & \mbox{(because $3+2=5$)}\\
&     &   & 2 &              & 2     & \upri & 2 & \mbox{(because $2+0=2$)}
\end{array}
\hspace*{5in}\]

Go on! Do it again!
\[\begin{array}{lr@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner&\\
& 0\, &   & 1 &   & 4 & \rightarrow  & 9     &       &   & \mbox{(because $4+5=9$)}\\
&     & 1 &   & 3 &   & 5            & \upri & 7     &   & \mbox{(because $5+2=7$)}\\
&     &   & 2 &   & 2 &              & 2     & \upri & 2 & \mbox{(because $2+0=2$)}
\end{array}
\hspace*{5in}\]

Why, you might ask, are we doing this? It's boring and repetitive and, well, mechanical.
And that's the point. It's an exercise in empathy: you're being the machine. It doesn't get to ask why, so shut up and compute another step!
\[\begin{array}{lr@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner&\\
& 0\, &   & 1 &   & 4 &   & 9 & \rightarrow & 16    &       &   & \mbox{(because $9+7=16$)}\\
&     & 1 &   & 3 &   & 5 &   & 7           & \upri & 9     &   & \mbox{(because $7+2=9$)}\\
&     &   & 2 &   & 2 &   & 2 &             & 2     & \upri & 2 & \mbox{(because $2+0=2$)}
\end{array}
\hspace*{5in}\]

Give it one last go! These numbers are getting dangerously large.
\[\begin{array}{lr@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner&\\
& 0\, &   & 1 &   & 4 &   & 9 &   & 16 & \rightarrow & 25    &       &   & \mbox{(because $16+9=25$)}\\
&     & 1 &   & 3 &   & 5 &   & 7 &    & 9           & \upri & 11    &   & \mbox{(because $9+2=11$)}\\
&     &   & 2 &   & 2 &   & 2 &   &  2 &             & 2     & \upri & 2 & \mbox{(because $2+0=2$)}
\end{array}
\hspace*{5in}\]

Now, let's see what's going on. Use the gap you left to number the stripes, starting from 0.
\[\begin{array}{r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{\qquad\qquad}l}
\ulcorner \backslash 0 &\backslash& 1 &\backslash& 2 &\backslash& 3 &\backslash& 4 &\backslash& 5 &&&& n\\
\hline
 \backslash&0\, & \backslash  & 1 & \backslash  & 4 & \backslash  & 9 & \backslash  & 16 & \backslash  & 25 &    &   & n\times n\\
&   \backslash  & 1 & \backslash  & 3 & \backslash  & 5 & \backslash  & 7 & \backslash   & 9 &  \backslash  & 11 &   & 2\times n + 1\\
&     &  \backslash & 2 & \backslash  & 2 & \backslash  & 2 & \backslash  &  2 & \backslash  &  2 &  \backslash  & 2 & 2}
\end{array}
\hspace*{5in}\]

Each stripe is labelled by the number of steps it took us to get there: no steps for the starting stripe, 0 1 2, one step for the next stripe, 1 3 2, two steps for 4 5 2, and so on. Looking at the rows instead of the stripes, we see the top row has the squares of the step count, the middle row steps through the odd numbers, and the bottom row is always 2. The difference between successive odd numbers is always 2, and the difference between successive squares is always the next odd number. Why is that? Is it your fault, or mine?

It's my fault, of course! You computed successive stripes by following an `adding up' rule which has nothing to do with being either odd or square, but I gave you the numbers, 0 1 2, to start from. You were the computer: I wrote the program. When I put that 2 at the end, I made sure the bottom row would always be 2. That means the middle row will keep going up by 2, so the fact that I kicked the middle row off with 1 made you compute the odd numbers. Meanwhile, let me show you a table:
\[\begin{array}{|c|c|c|c||c|}
\hline
1 & 2 & \cdots & n & n+1 \\
\hline
2 & 4 & \cdots & 2\timesn & 2\times(n+1) \\
\hline
\vdots & \vdots & \ddots & \vdots & \vdots \\
\hline
n & n\times2 & \cdots & n\times n & n\times(n+1) \\
\hline
\hline
n+1 & (n+1)\times 2 & \cdots & (n+1)\times n & (n+1)\times(n+1) \\
\hline
\end{array}\]
The number in each cell of the table tells you the number of cells in the rectangle which starts in the top left corner and has that cell as its bottom right corner. For the top left cell, that rectangle is exactly that cell. By the time we are $n$ cells right and $n$ cells down, we have enclosed $n\times n$ cells. To go down further by another one, we would enclose another $n$ cells. Likewise, to go right, we would enclose another $n$ cells. To do both, we enclose another $2\times n + 1$ cells: both of those blocks of $n$ and another $1$ in the bottom right corner. That is to say, to go from $n$ squared to $(n+1)$ squared, you increase by $2\times n + 1$, i.e., the next odd number.

So when I chose the 0, I chose that we were starting at $0\times 0$, and hence that the top row would give us the sequence of squares. We're using a \emph{general} mechanism of adding up numbers in a repetitive pattern to compute \emph{in this instance} the sequence of squares, by choosing to start \emph{in this instance} with the stripe 0 1 2. You could choose to use the same mechanism to compute a different sequence by kicking it off with a different stripe.

What happens if you choose 0 0 1, instead?
\[\begin{array}{r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}l@{}r@{}r@{\qquad\qquad}l}
\ulcorner \backslash 0 &\backslash& 1 &\backslash& 2 &\backslash& 3 &\backslash& 4 &\backslash& 5 &&&& n\\
\hline
 \backslash&0\, & \backslash  & 0 & \backslash  & 1 & \backslash  & 3 & \backslash  & 6 & \backslash  & 10 &    &   & \mbox{the total of the numbers less than $n$}\\
&   \backslash  & 0 & \backslash  & 1 & \backslash  & 2 & \backslash  & 3 & \backslash   & 4 &  \backslash   5 & &  & \mbox{$n$, being the total of $n$ copies of $1$}\\
&     &  \backslash & 1 & \backslash  & 1 & \backslash  & 1 & \backslash  &  1 & \backslash  &  1  \backslash  1 &&& 1}
\end{array}
\hspace*{5in}\]

What happens if you choose 0 0 2? That is to say, what happens if you double your opening stripe? Well, remember that our basic move consists of adding two numbers we already have. If you add the doubles of two numbers, you get double their sum: $2\times n +2\times m = 2\times(n + m)$. So everything doubles (apart from the step counts).
\[\begin{array}{r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}l@{}r@{}r@{\qquad\qquad}l}
\ulcorner \backslash 0 &\backslash& 1 &\backslash& 2 &\backslash& 3 &\backslash& 4 &\backslash& 5 &&&& n\\
\hline
 \backslash&0\, & \backslash  & 0 & \backslash  & 2 & \backslash  & 6 & \backslash  & 12 & \backslash  & 20 &    &   & \mbox{twice the total of the numbers less than $n$}\\
&   \backslash  & 0 & \backslash  & 2 & \backslash  & 4 & \backslash  & 6 & \backslash\;   & 8 &  \backslash   10 & &  & \mbox{$2\times n$, being the total of $n$ copies of $2$}\\
&     &  \backslash & 2 & \backslash  & 2 & \backslash  & 2 & \backslash  &  2 & \backslash  &  2  \backslash  2 &&& 2}
\end{array}
\hspace*{5in}\]

What happens if you choose the shorter stripe, 0 1? In effect, you chop off the top row from 0 0 1 and slide everything up and left.
\[\begin{array}{r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}r@{}l@{}r@{}r@{\qquad\qquad}l}
\ulcorner \backslash 0 &\backslash& 1 &\backslash& 2 &\backslash& 3 &\backslash& 4 &\backslash& 5 &&& n\\
\hline
\backslash  & 0 & \backslash  & 1 & \backslash  & 2 & \backslash  & 3 & \backslash   & 4 &  \backslash   5 & &  & \mbox{$n$, being the total of $n$ copies of $1$}\\
     &  \backslash & 1 & \backslash  & 1 & \backslash  & 1 & \backslash  &  1 & \backslash  &  1  \backslash  1 &&& 1}
...
Now, what happens if we take these last two runs and add together the numbers in corresponding positions, padding out the shorter stripes with 0 whenever we need to? Our starting stripe becomes
\[\begin{array}{rrrr}
& 0 & 0 & 2 \\
+ & 0 & 1 & \\
\hline
= & 0 & 1 & 2
\end{array}\]
which may seem familiar. Along the bottom, adding just gives copies of 2. In the middle, our even numbers each have a 1 added, giving the odd numbers. And along the top,
\[
0+0=0 \quad 0+1=1 \quad 2+2=4 \quad 6+3=9 \quad 12+4=16 \quad 20+5=25
\]
out pop the squares! Adding the input stripes has the effect of adding the whole tables, because we are adding together the same stuff, slightly rearranged in a way that does not make a difference. If we have $a+b$ in one table and $c+d$ in the other, we get $(a+c)+(b+d)$ in the combined table, which arrives at the same value as $(a+b)+(c+d)$.

You have been simulating \emph{the difference engine}, one of the earliest pieces of computing machinery designed by human beings. Feed in the input stripe for the sequence you want, and out comes the sequence! At least, it \emph{seems} to work. Do we believe our eyes? Can we come with a satisfactory \emph{explanation}? What is it about numbers that makes this machine do its thing? What are numbers anyway? How do we even start to talk about these ideas? What tools can we use to help us think about it? Let me invite you on a journey of invention.
...
Now, if you only make numbers this way, starting only with a lemon and holding onto only the most recently joined potato, your numbers will all have an important property. \textbf{If you follow the sticks down the number, you will eventually get to the lemon.} No sneaky moves, like making a circular model from potatoes and sticks, or always adding on the next potato just in time. What other ays of making spud and lemon models fail to give you legitimate numbers?
...
\end{enumerate}
But how do you know you \emph{can} count the spuds? How do you know that you won't just keep counting for ever? It's because you eventually get to the lemon. In some sense, our models are a \emph{tangible} form of counting, starting from a tangible, not to say tangy, representation of zero, with a potato representing every step of counting up. And if you only build numbers by counting up from zero, you're bound to reach zero, sooner or later, if you count back down.


\section{Declaring $\Number$ in $\ask$}

We've built a system in which you can play with these ideas. It's called $\ask$, because it's based on a fragment of the programming language \textbf{Haskell}. 

\begin{askism}[Everything Is New]
  You work with $\ask$ to write a document which contains ideas about data and programs and proofs. At the beginning of the document, $\ask$ has no preconceived ideas about which data exist. If you want numbers, you will have to invent them, as if they are a completely new idea.
\end{askism}

To invent numbers in $\ask$, you say something like

\[\begin{array}{r@{\;}r@{\;}l@{\quad}l}
\K{data}\;\Number & = & \Ze   & \cmt{start with a lemon}\\
                  & | & \Su\:\Number & \cmt{attach another spud}
\end{array}\]

Let's learn to read this, a little at a time.

\begin{askism}[Dash Dash Comment]
When you see two consecutive dashes, whatever comes after them is ignored, all the way to the end of the line. You can say whatever you like in a comment, and $\ask$ will ignore you. You do not have to write comments, but they may help you remember what you thought you were doing.
\end{askism}
...
\begin{askism}[Indent to Carry On]
  When the next (non-blank) line of $\ask$ code is strictly more indented than the line before, it is carrying on from the line before.
\end{askism}

So we could have written
\[
\K{data}\;\Number\;=\;\Ze\;|\;\Su\:\Number
\]
all on one line, and it would mean the same thing. We threw in a line break to make room for the comment about the lemon, and indented to make sure that $\ask$ knew we weren't done. Making the $|$ line up with the $=$ is not important (unless you judge by appearances); making the $|$ further to the right than the $\K{d}$ of $\K{data}$ is crucial.

We've talked about the shape of the code, but we should tell you what it means.

\begin{askism}[Data Type Is Made of What]
  The word $\K{data}$ is a special keyword in $\ask$. It means `I am inventing a whole new type of things!'. The name of the new type goes between $\K{data}$ and $=$, and it has to begin with a capital letter, but otherwise, you choose. After the $=$ sign, you list the choice of ways to make your new type of things, separated by $|$ signs.
\end{askism}

Our example $\K{data}$ declaration invents a new type of thing called $\Number$, and there are two ways to make things of that type.

\begin{askism}[Constructor Packs Stuff]
Each choice of `ways to make your new type of thing' is specified by giving a name beginning with a capital letter, called a `constructor' then a list of the types of the things you must pack along with the constructor to complete your new thing. The constructors names must be distinct.
\end{askism}

In our example, the constructors are $\Ze$ (for `zero') and $\Su$ (for `successor'). The $\Ze$, like our lemon, packs nothing; $\Su$, like our spud, packs another number, just as if it's attached with a stick.

\begin{askism}[Capitals for Vegetables]
  When you see a name in $\ask$ which begins with a capital letter, it's not a thing which does anything. It does not compute stuff. It just sits there being itself, and not being other things with different capital letter names, like our different vegetables stuck together with sticks. It's part of a type or some data, as specified in $\K{data}$ declarations.
\end{askism}

We can write down some examples of \emph{values} which belong to our type $\Number$:
\[
\Ze\qquad \Su\:\Ze\qquad \Su\:(\Su\:\Ze)\qquad \Su\:(\Su\:(\Su\:\Ze))
\qquad \Su\:(\Su\:(\Su\:(\Su\:\Ze)))
\]

In isolation, the $\Su$ and the $\Ze$ don't do anything. They just sit there, being parts of numbers. Let's be careful about how we read and write them.

\begin{askism}[Application Associates to the Left\footnote{`rather as we all did in the 60s', according to the logician, Roger Hindley}]
  When you see two things in $\ask$ separated by space, the left thing is `applied to' the right thing. When the things are constructors, that just means `packing'. When you see more than two things in a sequence separated by space, they group to the left, so $a\:b\:c$ means $(a\:b)\:c$, not $a\:(b\:c)$. You can use round brackets to override this grouping convention.
\end{askism}

That's why we wrote $\Su\:(\Su\:\Ze)$, not $\Su\:\Su\:\Ze$. The latter would be read as $(\Su\:\Su)\:\Ze$, which is nonsense, because $\Su$ packs up things of type $\Number$, and $\Su$ on its own is not a $\Number$ --- where's its lemon?

So we've invented $\Number$. And, like with the spuds and lemons, everything of type $\Number$ is made with finitely many layers of $\Su$ around a $\Ze$. There will never be any weird things of type $\Number$ that don't follow these rules, and in due course, we'll see how that enables us to \emph{compute} with numbers.

Let's practice making numbers.


\section{addition as substitution}

Adding physical spuds-and-lemons numbers is easy. If you're holding one of them in each hand, work your way along the left number until you reach the left lemon, then detach it, and stab the right number with the exposed stick, then work your way back out to the leftmost spud. In other words, \emph{substitute} the right \emph{number} for the left \emph{lemon}
\[\begin{array}{l@{\;\,}l}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{from veg to data}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

So far, we've been computing by dismantling trees of fruit and vegetables held together by pointy sticks. Sooner or later, it might be good to compute with a computer. I'm going to teach you a little bit of a programming language called \textbf{Haskell}\footnote{We teach more Haskell in years two and three, so this is a head start.} that's useful for programming with treelike structures. This little bit of Haskell is a system I cooked up, called \textbf{ask}\footnote{What is the rest of Haskell?}.

We start out with no fixed notions of which data exist, but we can invent new notions of data. Here's an example of how we do that, inventing \emph{numbers}.
...
\begin{itemize}
\item Notice that the second line is indented, relative to the first line. That means the second line continues what was started in the first line, rather than being a whole new thing.
\item The $\K{data}$ is a keyword. It introduces the invention of a new type of data.
\item The $=$ (`is') and the $|$ (`or') are special punctuation.
\item The $\D{N}$ between $\K{data}$ and $=$ is me \emph{choosing} a new name for a new type of data. I chose $\D{N}$ for number. To the right of $=$, I explain our (two) choices for how to make things of type $\D{N}$.
\item The $\D{Z}$ and $\D{S}$ immediately to the right of $=$ and $|$ are called `constructors', but not in the sense of Java. The $\C{Z}$ (`zest'? no, `zero') plays the role of $\lemon$ and the $\C{S}$ (`successor') plays the role of $\spud\;$.
\item To the right of each constructor, we see the \emph{types} of the things that constructor packs up. We're saying how many sticks there are, and what's at the other end of them. Like $\lemon$, $\C{Z}$ has no sticks. $\C{S}$ packs up one thing of type $\D{N}$, just as a $\spud\;$ connects to a number.
\item So $\D{N}$ is a new way of making \emph{types}, while $\C{Z}$ and $\C{S}$ are4 new ways to make \emph{things} of type $\D{N}$.
\end{itemize}

What is a \emph{type}? Firstly, types are things, just as numbers are things and formulae are things. Secondly, types have a job --- \emph{classification}. Not everything is classified as a number, but knowing something is classified as `an $\D{N}$' grants us some entitlements regarding that something: we're allowed to ask whether it is $\C{Z}$ or $(\C{S}\:n)$ for some $n$ which is also classified as `an $\D{N}$'.

Which brings me to a important point: Haskell is \emph{case-sensitive}, and so is \textbf{ask}. Names beginning with an uppercase letter play the role of \emph{vegetables}; names beginning with a lowercase letter play the role of \emph{variables}. Some variables stand for things we don't know, but if we know how they have been classified, that tells us there's some stuff we know how to do with those things.

The whole point of computer programming is to know how to do stuff, in general, with things which have not turned up yet, in particular. Types are a great way of making promises about what is going to work as long as the things which eventually turn up are appropriately classifiable. We promise to add any two things which are classified as numbers; we insist that you don't ask to add things which are not classified as numbers. Expectations are appropriately managed. If you \textbf{ask} a question which is meaningfully classifiable, you will get a meaningful answer.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{sums and differences}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\part{proof and refutation}
